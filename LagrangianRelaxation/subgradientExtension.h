...
#include "utils.h"
#include "instance.h"
#include "lowerProblemExtension.h"

void subgradientHarmonicExtension(
    ...
    lowerProblemExtension(inst, tk, v, WBest, h, S);
    long Wplus = getUBByHeuristic(inst, S);

    UB.clear();
    UB.push_back(Wplus);

    long it = 1;     // iteracja 1
    double C = 1.0;     // stała C
    ...
    while ((haltingLimit > 0 && it <= haltingLimit) || (haltingLimit < 0 && getClockMicro(clock) > -haltingLimit)) {
        ...
        double W;
        lowerProblemExtension(inst, tk, v, W, h, S);
...
void subgradientGammaFormulaExtension(
    const Instance & inst, long m,
...
void subgradientGammaRandomExtension(
    const Instance & inst, long m,
    double& WBest, std::vector<double> & v, std::vector<long> & S, std::vector<double> & trajectory, std::vector<long> & UB, std::vector<double> & alphas,
